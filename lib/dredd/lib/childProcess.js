...
function terminate(childProcess, options = {}, callback) {
  ...
  const onExit = () => {
    // POINT 9
    console.log('DJR: terminate.childProcess.onExit')
    terminated = true;
  ...
  function check() {
    // POINT 3
    console.log('DJR: terminate.check')
    if (terminated) {
    ...
    // POINT 4
    console.log('DJR: outside')
    if ((Date.now() - start) < timeout) {
      // POINT 5
      console.log('DJR: not timed out')
...
function spawn(...args) {
  ...
  childProcess.terminate = (options) => {
    // POINT 0
    // const customPsAux = crossSpawn.sync('ps aux').stdout.toString().trim();
  ...
  childProcess.on('exit', (exitStatus, signal) => {
    console.log('DJR: PROCESS EXIT', signal, exitStatus);
